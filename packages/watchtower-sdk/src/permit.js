...
export async function hashCalldata(calldata) {
    if (!calldata)
        return ZERO_BYTES32;
    const { keccak256 } = await import('viem');
    return keccak256(assertHex(calldata, 'calldata'));
}
export async function createExecutionHash(input) {
    if (input.executionHash)
        return assertBytes32(input.executionHash, 'executionHash');
...
export async function buildExecutionIntent(input) {
    const calldataHash = await hashCalldata(input.calldata);
    return {
        action: input.action.trim(),
        tokenAddress: assertAddress(input.tokenAddress, 'tokenAddress'),
...
export function verificationOptionsFromPolicy(policy = DefaultAuthorizationPolicy) {
    return {
        expectedSignerAddress: policy.signerAddress,
        expectedDomain: policy.domain,
    };
}
export function buildPermitMessage(authorization) {
    return {
        id: authorization.id,
...
export async function createPermitDigest(authorization) {
...
export async function verifyExecutionAuthorization(authorization, options = {}) {
    try {
