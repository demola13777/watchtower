...
1. Disable paid scan routes at the deployment layer.
...
2. Record the erroneous transaction hash, scan hash, chain ID, token, and cause.
3. Issue a corrected scan and attestation when appropriate, retaining the original record as an immutable audit artifact.
